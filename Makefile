...
.PHONY: sync-crds
sync-crds: manifests ## Regenerate the CRDs and derive the Helm chart CRD templates from them (config/crd/bases is the single source of truth).
	./hack/sync-crds.sh
...
K8S_CLUSTER_NAME := infisical-issuer-e2e
...
${INSTALL_YAML}: manifests kustomize
	...
	echo "Contents of build/kustomize:" && \
	ls -la build/kustomize && \
	$(KUSTOMIZE) build build/kustomize > $@ || (echo "Kustomize build failed"; exit 1)
	@echo "Install YAML created successfully"
...
CONTROLLER_TOOLS_VERSION ?= v0.16.1 # NOTE (dangtony98): Updated from v0.11.3
KIND_VERSION := 0.18.0
